...
export async function draftEmailForProspect(
  ...
  if (error || !prospect) return { ok: false, error: "Prospect not found" };
...
Write the email now. Respond with JSON: {"subject": "...", "body": "..."}.`;
  ...
  if (!subject || !body) return { ok: false, error: "Failed to parse model output", raw: text };
  ...
  if (updateErr) return { ok: false, error: `Failed to save draft: ${updateErr.message}` };
