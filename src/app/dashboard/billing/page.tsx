...
export default function BillingPage() {
  const [billingDocs, setBillingDocs] = useState<KBDocument[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/knowledge-base/me")
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">Manage billing FAQs and payment information for your AI receptionist</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs font-medium bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full">Coming Soon</span>
        </div>
        <p className="text-sm text-gray-500">
          Soon you&apos;ll be able to see a log of all calls where billing or payment questions were asked,
          along with what information the AI provided.
        </p>
      </div>
    </div>
  );
